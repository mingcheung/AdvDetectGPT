...
def predict(
    ...
    if image_path is None:
        return [(input, "There is no input image provided! Please upload your image and start the conversation.")]
    else:
...
with gr.Blocks(scale=4) as demo:
    gr.HTML("""<h1 align="center">AdvDetectGPT</h1>""")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            max_length = gr.Slider(0, 400, value=64, step=1.0, label="Maximum length", interactive=True)
            top_p = gr.Slider(0, 1, value=0.01, step=0.01, label="Top P", interactive=True)
            temperature = gr.Slider(0, 1, value=1.0, step=1.0, label="Temperature", interactive=True)
